...
module "ha-vnet" {
  source              = "../modules/vnet"
  vnet_name           = var.ha_vnet_name
  resource_group_name = module.common.resource_group_name
  location            = module.common.resource_group_location
  nsg_id              = module.ha-network-security-group.network_security_group_id
  address_space       = var.ha_address_space
  subnet_prefixes     = var.ha_subnet_prefixes
}

module "ha-network-security-group" {
  source              = "../modules/network-security-group"
  resource_group_name = module.common.resource_group_name
  security_group_name = "${module.common.resource_group_name}-ha-nsg"
  location            = module.common.resource_group_location
  security_rules = [
    {
...
resource "azurerm_public_ip" "ha-public-ip" {
  ...
  location            = module.common.resource_group_location
  resource_group_name = module.common.resource_group_name
  allocation_method   = module.ha-vnet.allocation_method
  sku                 = var.sku
  domain_name_label   = "${lower(var.cluster_name)}-${count.index + 1}-${random_id.randomId.hex}"
...
resource "azurerm_public_ip" "cluster-vip" {
  ...
  resource_group_name = module.common.resource_group_name
  allocation_method   = module.ha-vnet.allocation_method
  # should we expose it in variables
  sku                 = var.sku
  domain_name_label   = "${lower(var.cluster_name)}-vip-${random_id.randomId.hex}"
  public_ip_prefix_id = var.use_public_ip_prefix ? (var.create_public_ip_prefix ? azurerm_public_ip_prefix.public_ip_prefix[0].id : var.existing_public_ip_prefix_id) : null
}
...
resource "azurerm_network_interface" "nic_vip" {
  ...
  azurerm_public_ip.ha-public-ip]
  ...
  ip_configuration {
    name                          = "ipconfig1"
    primary                       = true
    subnet_id                     = module.ha-vnet.vnet_subnets[0]
    private_ip_address_allocation = module.ha-vnet.allocation_method
    private_ip_address            = cidrhost(module.ha-vnet.subnet_prefixes[0], 5)
    public_ip_address_id          = azurerm_public_ip.ha-public-ip.0.id
  }
  ip_configuration {
    name                          = "cluster-vip"
    subnet_id                     = module.ha-vnet.vnet_subnets[0]
    primary                       = false
    private_ip_address_allocation = module.ha-vnet.allocation_method
    private_ip_address            = cidrhost(module.ha-vnet.subnet_prefixes[0], 7)
...
resource "azurerm_network_interface" "ha-nic" {
  depends_on = [
    azurerm_public_ip.ha-public-ip,
  ...
  ip_configuration {
    ...
    subnet_id                     = module.ha-vnet.vnet_subnets[0]
    private_ip_address_allocation = module.ha-vnet.allocation_method
    private_ip_address            = cidrhost(module.ha-vnet.subnet_prefixes[0], 6)
    public_ip_address_id          = azurerm_public_ip.ha-public-ip.1.id
  }
  lifecycle {
    ignore_changes = [
      # Ignore changes to ip_configuration when Re-applying, e.g. because a cluster failover and associating the cluster- vip with the other member.
...
resource "azurerm_network_interface_backend_address_pool_association" "nic_lb_association" {
  depends_on              = [azurerm_network_interface.ha-nic, azurerm_lb_backend_address_pool.frontend-lb-pool]
  network_interface_id    = azurerm_network_interface.ha-nic.id
  ip_configuration_name   = "ipconfig1"
  backend_address_pool_id = azurerm_lb_backend_address_pool.frontend-lb-pool.id
}

resource "azurerm_network_interface" "ha-nic1" {
  depends_on = [
  ...
  ip_configuration {
    name                          = "ipconfig2"
    subnet_id                     = module.ha-vnet.vnet_subnets[1]
    private_ip_address_allocation = module.ha-vnet.allocation_method
    private_ip_address            = cidrhost(module.ha-vnet.subnet_prefixes[1], count.index + 5)
  }
}

resource "azurerm_network_interface_backend_address_pool_association" "nic1_lb_association" {
  depends_on              = [azurerm_network_interface.ha-nic1, azurerm_lb_backend_address_pool.backend-lb-pool]
  count                   = 2
  network_interface_id    = azurerm_network_interface.ha-nic1[count.index].id
...
resource "azurerm_public_ip" "public-ip-lb" {
  ...
  allocation_method   = module.ha-vnet.allocation_method
  sku                 = var.sku
  domain_name_label   = "${lower(var.cluster_name)}-${random_id.randomId.hex}"
  public_ip_prefix_id = var.use_public_ip_prefix ? (var.create_public_ip_prefix ? azurerm_public_ip_prefix.public_ip_prefix[0].id : var.existing_public_ip_prefix_id) : null
}
...
resource "azurerm_lb" "backend-lb" {
  ...
  frontend_ip_configuration {
    ...
    subnet_id                     = module.ha-vnet.vnet_subnets[1]
    private_ip_address_allocation = module.ha-vnet.allocation_method
    private_ip_address            = cidrhost(module.ha-vnet.subnet_prefixes[1], 4)
  }
}
...
resource "azurerm_virtual_machine" "vm-instance-availability-set" {
  depends_on = [
    azurerm_network_interface.ha-nic,
    azurerm_network_interface.ha-nic1,
  ...
  os_profile {
    ...
    custom_data = templatefile("${path.module}/ha-init.sh", {
      installation_type              = var.ha_installation_type
      allow_upload_download          = module.common.allow_upload_download
      os_version                     = module.common.os_version
      template_name                  = var.ha_template_name
      template_version               = var.ha_template_version
      ...
      virtual_network                = module.ha-vnet.vnet_name
...
resource "azurerm_virtual_machine" "vm-instance-availability-zone" {
  ...
  os_profile {
    ...
    custom_data = templatefile("${path.module}/ha-init.sh", {
      ...
      virtual_network                = module.ha-vnet.vnet_name
      cluster_name                   = var.cluster_name
      external_private_addresses     = cidrhost(module.ha-vnet.subnet_prefixes[0], 7)
      enable_custom_metrics          = var.enable_custom_metrics ? "yes" : "no"
      admin_shell                    = var.admin_shell
      smart_1_cloud_token            = count.index == 0 ? var.smart_1_cloud_token_a : var.smart_1_cloud_token_b
      serial_console_password_hash   = var.serial_console_password_hash
